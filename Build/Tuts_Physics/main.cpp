...
void PrintStatusEntries()
{
	...
	{		
		...
		NCLDebug::AddStatusEntry(status_color_debug, "BroadPhase Shapes         : %s [B]", (drawFlags & DEBUGDRAW_FLAGS_BROADPHASESHAPES) ? "Enabled " : "Disabled");
		NCLDebug::AddStatusEntry(status_color_debug, "Manifolds         : %s [V] - Tut 5+", (drawFlags & DEBUGDRAW_FLAGS_MANIFOLD) ? "Enabled " : "Disabled");
		NCLDebug::AddStatusEntry(status_color_debug, "BP OCT         : %s [L] - Tut 5+", (drawFlags & DEBUGDRAW_FLAGS_BROADPHASEOCT) ? "Enabled " : "Disabled");
	...
	if (draw_performance)
	{
		...
		NCLDebug::AddStatusEntry(status_color_performance, "BP Collision Pairs:  %i", PhysicsEngine::Instance()->getNumCols());
...
void HandleKeyboardInputs()
{
	...
	if (Window::GetKeyboard()->KeyTriggered(KEYBOARD_B))
		drawFlags ^= DEBUGDRAW_FLAGS_BROADPHASESHAPES;
