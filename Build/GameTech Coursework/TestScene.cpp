...
void TestScene::OnInitializeScene()
{
	...
	m_pPlayer  = new GameObject("Player1");
	m_pPlayer->SetRender(new RenderNode(m_MeshPlayer));
	m_pPlayer->Render()->SetBoundingRadius(1.0f);
	m_pPlayer->Render()->SetColor(Vector4(1.0f, 1.0f, 1.0f, 1.0f));

	m_pPlayer->SetPhysics(new PhysicsNode());
	m_pPlayer->Physics()->SetPosition(Vector3(0.0f, 0.5f, 0.0f));
	//player->Physics()->SetCollisionShape(new CuboidCollisionShape(Vector3(0.5f, 0.5f, 1.0f)));
	m_pPlayer->Physics()->SetBroadphaseShape(new SphereCollisionShape(Vector3(0.5f, 0.5f, 1.0f).Length()));
	m_pPlayer->Physics()->AddColShape(new CuboidCollisionShape(Vector3(0.3f, 0.3f, 0.3f), Vector3(0.0f, 0.3f, -0.4f)));
	m_pPlayer->Physics()->AddColShape(new CuboidCollisionShape(Vector3(0.3f, 0.3f, 0.3f), Vector3(0.0f, 0.1f, 0.1f)));
	this->AddGameObject(m_pPlayer);
}
