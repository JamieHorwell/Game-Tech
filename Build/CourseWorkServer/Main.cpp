...
NetworkBase server;
GameTimer timer;
ServerPacketProcessor packetProcessor;
...
int main(int arcg, char** argv)
{
	...
	packetProcessor.setNetwork(server);
	...
	while (true)
	{
		...
		server.ServiceNetwork(dt, [&](const ENetEvent& evnt)
		{
			...
			case ENET_EVENT_TYPE_RECEIVE:
				packetProcessor.processPacketInstruction(evnt.packet);
		...
		if (accum_time >= UPDATE_TIMESTEP)
		{
			...
			accum_time = 0;
			if (packetProcessor.getAvatarSpawned()) {
				packetProcessor.updateAvatar(UPDATE_TIMESTEP);
			}
			if (packetProcessor.getMazeSpawned()) {
				packetProcessor.updateRaptors(UPDATE_TIMESTEP);
			}
