...
public:
	...
	CuboidCollisionShape(const Vector3& halfdims, Vector3 localOffset = Vector3(0,0,0));
	...
	virtual void GetCollisionAxes(const CollisionShape* otherShape, std::vector<Vector3>& out_axes) const override;


	virtual Vector3 GetClosestPoint(const Vector3& point) const override;

	virtual Vector3 getTrueClosestPoint(const Vector3& point) const override;

	virtual Vector3 getWorldTransform() const override;
