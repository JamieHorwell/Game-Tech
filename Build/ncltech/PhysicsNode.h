...
struct CollNode {
	...
	CollNode(Vector3 offset, CollisionShape* shape) : offSet(offset), shape(shape) {}
};
...
public:
	...
	virtual ~PhysicsNode()
	{
		...
		SAFE_DELETE(sphereBroadShape);
		for (int i = 0; i < collNodes.size(); ++i) {
			if (collNodes[i] != nullptr) {
				//SAFE_DELETE(collNodes[i]);
			}
	...
	inline const int			GetNumColShapes() const { return numOfColShapes; }

	inline  std::vector<CollisionShape*>& getCollNodes()  { return collNodes; }
	...
	inline void SetNumberColShapes(int num) { this->numOfColShapes = num; }

	inline void AddColShape(CollisionShape* collShape) { collNodes.push_back(collShape); collShape->SetParent(this); };
...
protected:
	...
	int numOfColShapes = 1;
	std::vector<CollisionShape*> collNodes;
