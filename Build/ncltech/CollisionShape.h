...
//<----- USED BY COLLISION DETECTION ----->
	...
	virtual void GetCollisionAxes(const CollisionShape* otherShape, std::vector<Vector3>& out_axes) const = 0;
	...
	Vector3 getLocalTransform() { return localOffset; };
	void setLocalTransform(Vector3 localTransform) { localOffset = localTransform; };

	virtual Vector3 getWorldTransform() const = 0;

	virtual Vector3 getTrueClosestPoint(const Vector3& point) const = 0;
