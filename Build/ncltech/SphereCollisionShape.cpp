...
{
	m_Radius = 1.0f;
	localOffset = Vector3(0, 0, 0);
}

SphereCollisionShape::SphereCollisionShape(float radius, Vector3 localOffset)
{
	m_Radius = radius;
...
void SphereCollisionShape::GetCollisionAxes(const PhysicsNode* otherObject, std::vector<Vector3>& out_axes) const
{
	...
	*/
	Vector3 dir = (otherObject->GetPosition() - (Parent()->GetPosition() + localOffset)).Normalise();
...
void SphereCollisionShape::GetCollisionAxes(const CollisionShape * otherShape, std::vector<Vector3>& out_axes) const
{
	Vector3 dir = (otherShape->getWorldTransform() - getWorldTransform()).Normalise();

	Vector3 p1 = getWorldTransform();
	Vector3 p2 = otherShape->getTrueClosestPoint(p1);
...
Vector3 SphereCollisionShape::getTrueClosestPoint(const Vector3 & point) const
{
	Vector3 diff = (point - getWorldTransform()).Normalise();
	return getWorldTransform() + diff * m_Radius;
}

Vector3 SphereCollisionShape::getWorldTransform() const
{
	return localOffset + Parent()->GetPosition();
}
...
{
	out_min = Parent()->GetPosition() + localOffset - axis * m_Radius;
	out_max = Parent()->GetPosition() + localOffset + axis * m_Radius;
}
//-------------
	...
	std::vector<Plane>& out_adjacent_planes) const
{
	...
	out_face.push_back(Parent()->GetPosition() + localOffset + axis * m_Radius);
	out_normal = axis;	
}

void SphereCollisionShape::DebugDraw() const
{
	Vector3 pos = Parent()->GetPosition() + localOffset;
