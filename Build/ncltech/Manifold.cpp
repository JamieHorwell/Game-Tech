...
{	
	...
	if (constraintMass > 0.0f) {
		...
		//Ensure collision never solves itself backwards.
		//friction force never exceeds the force applied by the main collision force.
	...
	if (tangent_len > 1e-6f) {
		...
		if (frictionalMass > 0.0f) {
			...
			//stop friction from ever being more than frictionCoef* normal resolution
