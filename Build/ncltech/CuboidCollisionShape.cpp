...
CuboidCollisionShape::CuboidCollisionShape(const Vector3& halfdims, Vector3 localOffset)
{
	halfDims = halfdims;
...
void CuboidCollisionShape::GetCollisionAxes(const CollisionShape * otherShape, std::vector<Vector3>& out_axes) const
{
	Matrix3 objOrientation = Parent()->GetOrientation().ToMatrix3();
	out_axes.push_back(objOrientation * Vector3(1.0f, 0.0f, 0.0f)); //X - Axis
	out_axes.push_back(objOrientation * Vector3(0.0f, 1.0f, 0.0f)); //Y - Axis
...
Vector3 CuboidCollisionShape::GetClosestPoint(const Vector3& point) const
{
	...
	Matrix4 wsTransform = Parent()->GetWorldSpaceTransform() * Matrix4::Translation(localOffset) * Matrix4::Scale(halfDims);
...
Vector3 CuboidCollisionShape::getTrueClosestPoint(const Vector3 & point) const
{
	//Iterate over each edge and get the closest point on any edge to point p.

	// Build World Transform
	Matrix4 wsTransform = Matrix4::Translation(getWorldTransform()) * Matrix4::Scale(halfDims);
...
Vector3 CuboidCollisionShape::getWorldTransform() const
{
...
void CuboidCollisionShape::DebugDraw() const
{
	...
	Matrix4 transform = Parent()->GetWorldSpaceTransform() * Matrix4::Translation(localOffset) *  Matrix4::Scale(halfDims);
